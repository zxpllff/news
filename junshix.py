...
def get_articles(listUrl):              
    ...
    for index,value in enumerate(lists):
        ...
        cursor.execute('insert into articles (title, type_id, text, imgs, published_time, resoure, url, create_time) values ( %s, %s, %s, %s, %s, %s, %s, %s)', [item[0], 3, item[3], articlImg, item[1], item[2], articleUrl, create_time])
        cursor.rowcount
        # 提交事务:
        conn.commit()
        cursor.close()

#listUrl = 'http://temp.163.com/special/00804KVA/cm_war_02.js?callback=data_callback'
print ('————————程序开始————————')
i = 1
while i>=1 and i <= 3:
    if i==1:
        ...
        num = '_0' + str(i)
    listUrl = 'http://temp.163.com/special/00804KVA/cm_war%(num)s.js?callback=data_callback' % {'num':num}
    print('————————文章列表————————' + listUrl)
